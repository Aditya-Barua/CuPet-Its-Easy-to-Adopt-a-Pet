...
const PostPetSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      setFormError(true);
      if (!isFeeValid(fee)) setFeeError(true);
      return;
    }
  ...
  const updateFee = (e) => {
    const feeInput = e.target.value;
    if (isFeeValid(feeInput)) {
      setFee(feeInput);
      setFeeError(false); 
    } else {
      setFeeError(true);
  ...
  return (
    <section className="post-pet-section">
      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        {formError && (
          <p className="error-message">Please provide a valid fee.</p>
        )}
